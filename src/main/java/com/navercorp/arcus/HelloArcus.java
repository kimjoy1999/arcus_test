...
public class HelloArcus {
	...
	public HelloArcus(String arcusAdmin, String serviceCode) {
		...
		//
		// 정리하면 arcusAdmin과 serviceCode의 조합을 통해 유일한 캐시 서버들의 집합을 얻어 연결할 수 있는
		// 것입니다.
	...
	public String listenHello() {
		...
		try {
			future = this.arcusClient.asyncBopGet(key, 1000L, 2000L, filter, 0, 1000, false, false); // 1000~2000까지의
																										// Element를
																										// 일괄
																										// 조회한다.
		} catch (Exception e) {
			...
			if (result != null) {
				...
				for (Map.Entry<Long, Element<Object>> entry : result.entrySet()) {
					...
					if (entry.getKey() != Long.parseLong((String) entry.getValue().getValue())) {
						resultString = "error) key:" + entry.getKey() + ", value:"
